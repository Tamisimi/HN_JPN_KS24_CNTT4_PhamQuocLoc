...
-- Câu 4:
create view View_StudentCountByDept as select d.DeptName, count(s.StudentID) as TotalStudents
from Department d left join Student s on d.DeptID = s.DeptID group by d.deptName;
...
begin 
	select s.studentID , s.FullName , e.Score from Student s
    join enrollment e on s.StudentID = e.StudentID
    where e.CourseID = p_CourseID and e.Score = (select max(Score) from Enrollment where CourseID = p_CourseID);
end //
delimiter ;
